> 提取自 Qoder 内置 skill：`create-skill`（通过 `Skill` 工具在会话内加载后原文落盘）。
> Base directory (in-runtime): `.\builtin:builtin_skills\create-skill`
> 磁盘未提供可读的 SKILL.md（随 Qoder IDE 打包），故本文件是唯一的可查阅副本。

---

# Creating Skills in Qoder

This skill guides you through creating effective Agent Skills for Qoder. Skills are markdown files that teach the agent to perform specific tasks: reviewing PRs using team standards, generating commit messages in a preferred format, querying database schemas, or any specialized workflow.
...
2. **Target location**: Should this be a personal skill (~/.qoder-cn/skills/) or project skill (.qoder/skills/)?
...
- "Where should this skill be stored?" with options like ["Personal (~/.qoder-cn/skills/)", "Project (.qoder/skills/)"]
...
| Type | Path | Scope |
|------|------|-------|
| Personal | ~/.qoder-cn/skills/skill-name/ | Available across all your projects |
| Project | .qoder/skills/skill-name/ | Shared with anyone using the repository |
